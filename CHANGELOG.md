...
* Added a new `NavigationMapView.trafficOverrideRoadClasses` property. For any road class in it all route segments with an `CongestionLevel.unknown` traffic congestion level and a matching `MapboxDirections.RoadClass` will be replaced with the `CongestionLevel.low` congestion level. ([#2741](https://github.com/mapbox/mapbox-navigation-ios/pull/2741) 
